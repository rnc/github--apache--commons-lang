...
echo "Cache ${JBS_DISABLE_CACHE+} and URL ${CACHE_URL}"
env
...
if [ -n "" ]; then
  ...
  mvn -B -e -s "/var/workdir/software/settings/settings.xml" -t "/var/workdir/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="2.5" | tee /var/workdir/workspace/logs/enforce-version.log
fi
...
cp -r "${HOME}"/.m2/repository/* /var/workdir/workspace/build-info
