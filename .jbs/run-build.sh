...
cp -r /root/project/source/maven-artifacts/* "$HOME/.m2/repository/" || true
...
if [ -n "" ]; then
  echo "Setting version to 2.5 to match enforced version"
  mvn -B -e -s "/root/software/settings/settings.xml" -t "/root/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="2.5" | tee /root/project/logs/enforce-version.log
fi
...
if [ ! -d /root/project/source-archive ]; then
    cp -ar /root/project/source /root/project/source-archive
    ls -la /root/project/source-archive/
fi
